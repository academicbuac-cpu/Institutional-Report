...
import { PenTool, Tent, HeartHandshake, TreePine } from 'lucide-react';
const icons = [PenTool, Tent, HeartHandshake, TreePine];

export default function SabahActivitiesSlide() {
  const { ref, isInView } = useInView(0.2);
  const { data } = useData();
  const d = data.sabahActivities;
  return (
    <section id="sabah-activities" ref={ref} className="min-h-screen flex items-center justify-center bg-college-cream py-16 px-4">
      <div className="max-w-6xl mx-auto w-full">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
          {d.items.map((item, idx) => { const Icon = icons[idx % icons.length]; return (
            <div key={idx} className={`bg-white rounded-xl p-8 shadow-sm border-t-4 border-college-gold hover:shadow-lg transition-all duration-700 group ${isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'}`} style={{ transitionDelay: `${400 + idx * 150}ms` }}>
