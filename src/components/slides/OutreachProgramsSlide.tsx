...
import { Mic, Church, UsersRound } from 'lucide-react';
const icons = [Mic, Church, UsersRound];

export default function OutreachProgramsSlide() {
  const { ref, isInView } = useInView(0.2);
  const { data } = useData();
  const d = data.outreach;
  return (
    <section id="outreach" ref={ref} className="min-h-screen flex items-center justify-center bg-college-cream-light py-16 px-4">
